...
print('train_models_baseline.py', args.bert_type, args.dataset, args.alpha)
...
model = NimbleBert(
        model,
        name = '{}-double-head-unfrozen-{}-a{}'.format(args.bert_type.split('/')[-1], args.dataset, str(args.alpha)),
    ).cuda()
...
model.fit(train_loader, valid_loader, epochs=100, warm_up=256, auto_save_path='./trained_models', 
