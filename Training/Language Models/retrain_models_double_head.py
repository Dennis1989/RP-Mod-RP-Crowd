...
print('retrain_models_baseline.py', args.bert_type, args.dataset, args.alpha)
...
model = BertForPreTraining.from_pretrained(args.bert_type)
...
train_d = TextDatasetFineTuning('../../Dataset/Text-Data/{}.csv'.format(args.dataset), tokenizer, 'ten_folds', 
                                [0,1,2,3,4,5,6,7], include_mlm=True,
                                mlm_probability=.15, padding='max_length', truncation=True, max_length=512)
valid_d = TextDatasetFineTuning('../../Dataset/Text-Data/{}.csv'.format(args.dataset), tokenizer, 'ten_folds',
                                [8,9], include_mlm=True, 
...
model.cls.predictions.decoder = nn.Sequential(
    nn.Linear(768, tokenizer.vocab_size),
    nn.LogSoftmax(-1)
)

model.cls.seq_relationship = nn.Sequential(
    nn.Linear(768, 384),
    nn.GELU(),
    nn.Linear(384, 2),
    nn.LogSoftmax(-1)
...
model = NimbleBert(
        model,
        name = '{}-double-head-unfrozen-{}-a{}-4'.format(args.bert_type.split('/')[-1], args.dataset, str(args.alpha)),
    ).cuda()

model.set_optimizer('AdamW', lr=1e-4, weight_decay=0.01)
model.set_scheduler('MultiplicativeLR', lr_lambda, last_epoch=-1)
model.set_loss('Transformer.DoubleLoss', args.alpha, use_log=True)
model.add_metric('Bert.Acc')
model.add_metric('Bert.AuC', use_log=True)
model.freeze()

model.fit(train_loader, valid_loader, epochs=100, warm_up=256, auto_save_path='./trained_models_rep_1-4', 
          gradually_unfreeze=5, disabled_tqdm=True)
...
## Calc Predictions
model = torch.load('trained_models_rep_1-4/{}_best.pt'.format(NAME)).cuda()
probs = []
decisions = []
y_trues = []
...
## Save Pickle
with open('../../Evaluation/Repeated Language Model Results/{}_best.pickle'.format(NAME), 'wb') as f:
