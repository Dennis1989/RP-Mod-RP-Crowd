...
model = NimbleBert(
        model,
        name = '{}-baseline-unfrozen-{}-{}'.format(args.bert_type.split('/')[-1], args.dataset, args.rep),
    ).cuda()
...
model.fit(train_loader, valid_loader, epochs=100, warm_up=256, auto_save_path='./trained_models_rep_1-4', disabled_tqdm=True)
